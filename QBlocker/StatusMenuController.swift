...
class StatusMenuController: NSObject, NSMenuDelegate {
    ...
    func menuWillOpen(_ menu: NSMenu) {
        statusMenu.item(at: 0)?.title = String(format: "%d Quits Blocked", KeyListener.sharedKeyListener.accidentalQuits)
        statusMenu.item(at: 3)?.state = AtLogin.enabled ? .on : .off
    }
